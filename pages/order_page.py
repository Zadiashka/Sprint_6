...
class OrderPage(BasePage):
    ...
    def select_metro(self, metro: str) -> None:
        # вводим текст и выбираем первый подходящий вариант
        self.fill(self.METRO_INPUT, metro)
        # дождёмся появления опций и выберем первую, содержащую текст
        options = self.find_all(self.METRO_OPTIONS, timeout=5)
        for opt in options:
            if metro.lower() in opt.text.lower():
                opt.click()
                return
        # если не нашли — кликнем по первой опции
        if options:
            options[0].click()
    ...
    def set_date(self, day: int) -> None:
        # откроем календарь и выберем день по видимому тексту
    ...
    def choose_rental_days(self, days_text: str) -> None:
        ...
        for opt in options:
            if opt.text.strip().lower() == days_text.strip().lower():
                opt.click()
                return
        # если не нашли — кликнем по первой
        if options:
            options[0].click()
    ...
    def choose_color(self, color_text: str) -> None:
        ...
        for lbl in labels:
            if color_text.lower() in lbl.text.lower():
                lbl.click()
                return
        # если не нашли — кликнем по первому
        if labels:
            labels[0].click()
